...
async def test_simple_workflow():
    ...
    # 初始用户输入消息
    initial_message = "ロボットが、ティーチングポイントP3およびP4で定義された固定の部品供給ステーションから原材料または一次加工品を掴み取り（部品供給）、次にその部品を第二ステーション（ステーション2）へ搬送し、そこでP15で部品を掴んでからRz軸を90度から-90度へ180度回転させ、別の位置P18へ正確に再配置する中間処理を行い、最後に加工済みまたは処理済みと見なされた部品を最終的な設置エリア（ティーチINGポイントP11およびP12で定義）へ搬送して正確に配置することで、一つの完全な作業サイクルを完了させます。"
