...
from ..types import RouteDecision # Corrected import path
...
1.  **流程图编辑 (planner)**: 用户想要创建、修改、删除流程图的节点、边，或者询问与流程图结构相关的问题。
    例如："创建一个开始节点", "将节点A连接到节点B", "删除那个判断节点", "流程图现在是什么样的？"
2.  **示教点操作 (teaching)**: 用户想要保存、查询、修改或删除特定的坐标点或位置信息。
    例如："记录当前位置为P1", "P1的坐标是多少?", "更新P1到新的位置", "删除示教点P2"
3.  **信息咨询 (ask_info)**: 用户在询问你的能力、工作范围、如何与你交互，或者寻求一般性的帮助和建议。
    例如："你能做什么？", "我应该怎么告诉你创建流程图？", "你好" (这类无明确任务的通常也归为此类，可以给出问候和引导)
