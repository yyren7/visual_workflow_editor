...
def run_migrations_online() -> None:
    ...
    # print(f"Config Section Name: {config.config_ini_section}")
    # print(f"Config Section Content: {alembic_config_section}")
    # print(f"sqlalchemy.url from config: {config.get_main_option('sqlalchemy.url')}")
    # <<< 结束移除 >>>

    connectable = engine_from_config(
        config.get_section(config.config_ini_section, {}), # 恢复原始调用
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )

    # <<< 移除调试打印 >>>
    # print(f"Engine created: {connectable}")
