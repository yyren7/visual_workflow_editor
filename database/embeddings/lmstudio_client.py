...
class LMStudioClient:
    """LMStudio API客户端，用于调用外部embedding模型"""
    ...
    def create_embedding(self, text: str) -> List[float]:
        """
        调用LMStudio API创建文本的embedding向量
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "data" in result and len(result["data"]) > 0:
                    ...
                    logger.error(f"无法从LMStudio响应中提取embeddings: {result}")
                    raise ValueError("无法从响应中提取embeddings")
            else:
                logger.error(f"LMStudio API批量请求失败: {response.status_code}, {response.text}")
                raise ValueError(f"API批量请求失败: {response.status_code}")
                
        except Exception as e:
            logger.error(f"批量调用LMStudio API创建embeddings时出错: {str(e)}")
            raise 