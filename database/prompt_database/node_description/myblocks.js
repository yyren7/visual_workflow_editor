...
class BlockUtility {
  constructor() {
    ...
    this.teachingJson = [];
    this.flagNames = [];
    this.flagNamesReadOnly = [];
    this.flagParams = [];
    this.numberNames = [];
    this.numberNamesReadOnly = [];
...
class BlockFlow {
    ...
    for (let i = 0; i < blocks.length; i++) {
        ...
        if (block.getAttribute("id") === blockId) {
            ...
            while (parentBlock) {
              ...
              const parentHasDisabled = parentBlock.getAttribute("disabled-reasons");
              if(parentBlockId){
  ...
  convertXMLtoArray(xml) {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      const hasDisabled = block.getAttribute("disabled-reasons");
...
class BlockForm {
  ...
  setBlockParameter(numberJson, flagJson) {
    // 数値パラメータリスト作成
    let numberNames = [];
    let numberNamesReadOnly = [];
    for (let key in numberJson) {
      numberNamesReadOnly.push([numberJson[key]['name'], key]);
    }
    numberNames = numberNamesReadOnly.slice(0, 300);
    this.numberNames = numberNames;
    this.numberNamesReadOnly = numberNamesReadOnly;


    // フラグパラメータリスト作成
    let flagNames = [];
    let flagNamesReadOnly = [];
    for (let key in flagJson) {
      flagNamesReadOnly.push([flagJson[key]['name'], key]);
    }
    flagNames = flagNamesReadOnly.slice(0, 300);
    this.flagNames = flagNames;
    this.flagNamesReadOnly = flagNamesReadOnly;

    // 見た目変更
    this.workspace.getAllBlocks().forEach(block => {
      // 数値
      if (block.type === 'wait_timer') {
        this.updateDropdownParam(block, 'name', numberNamesReadOnly, numberJson);
      }
      ...
      // フラグ
        ...
        this.updateDropdownParam(block, 'name', flagNamesReadOnly, flagJson);
      }
    });
  }
  ...
  setBlockIOName(robotIONames, externalIOInfo, externalIONames) {
      // 表示名設定
      ...
      this.externalMakers = externalIOInfo[0];
      this.externalDevices = externalIOInfo[1];
      ...
      local_wk.getAllBlocks().forEach(block => {
        ...
        else if (block.type === 'connect_external_io') {
          this.updateDropdownIO(block, 'device_name', this.externalDevices);
          this.updateDropdownIO(block, 'external_maker', this.externalMakers);
        }
      });
  }   
  ...
  defineBlockForm() {
      ...
      Blockly.Blocks['set_output'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.robotOutputNames || self.robotOutputNames.length === 0) {
            self.robotOutputNames = [['Reload page.', 'none']]; 
          }
      ...
      Blockly.Blocks['set_output_until'] = {
        init: function() {
          ...
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.robotInputNames || self.robotInputNames.length === 0) {
            self.robotInputNames = [['Reload page.', 'none']]; 
          }
      ...
      Blockly.Blocks['set_output_during'] = {
        init: function() {
          ...
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.numberNamesReadOnly || self.numberNamesReadOnly.length === 0) {
            self.numberNamesReadOnly = [['Reload page.', 'none']]; 
          }
          ...
          this.appendDummyInput()
              ...
              .appendField("during")
              .appendField(new Blockly.FieldDropdown(self.numberNamesReadOnly), "name")
              .appendField("msec")
          this.setInputsInline(true);
          this.setPreviousStatement(true, "process");
          this.setNextStatement(true, "process");
  ...
  // --- Moveパラメータ設定の共通定義  ---
  // オプションリストを生成するヘルパー関数
  const generateNumericOptions = (prefix, count, noOptionLabel, noOptionValue = "none") => {
    const options = [[noOptionLabel, noOptionValue]];
    for (let i = 1; i <= count; i++) {
      options.push([`${prefix} No.${i}`, i.toString()]);
    }
    return options;
  };

  const PALLET_OPTIONS = generateNumericOptions('pallet', 10, 'no pallet');
  const CAMERA_OPTIONS = generateNumericOptions('camera', 10, 'no camera');

  
  const MOVE_PARAM_DEFINITIONS = [
    { name: 'execution_mode', label: 'Move Mode:', options: [["WAIT", "wait"],["CONT", "cont"]], default: 'wait', newLineInMutator: false },
    { name: 'control_x',  label: 'Enabled Axs:', options: [["X","enable"],["-","disable"]], default: 'enable', isAxis: true, axisLabel: 'X', newLineInMutator: true }, // 最初の軸の前で改行
    { name: 'control_y',  label: '', options: [["Y","enable"],["-","disable"]], default: 'enable', isAxis: true, axisLabel: 'Y', newLineInMutator: false },
    { name: 'control_z',  label: '', options: [["Z","enable"],["-","disable"]], default: 'enable', isAxis: true, axisLabel: 'Z', newLineInMutator: false },
    { name: 'control_rx', label: '', options: [["Rx","enable"],["-","disable"]], default: 'enable', isAxis: true, axisLabel: 'Rx', newLineInMutator: false },
    { name: 'control_ry', label: '', options: [["Ry","enable"],["-","disable"]], default: 'enable', isAxis: true, axisLabel: 'Ry', newLineInMutator: false },
    { name: 'control_rz', label: '', options: [["Rz","enable"],["-","disable"]], default: 'enable', isAxis: true, axisLabel: 'Rz', newLineInMutator: false },
    { name: 'pallet_list', label: 'Pallet:', options: PALLET_OPTIONS, default: 'none', newLineInMutator: true }, // Palletの前で改行
    { name: 'camera_list', label: 'Camera:', options: CAMERA_OPTIONS, default: 'none', newLineInMutator: true } // Cameraの前で改行
  ];
  // --- ここまでパラメータ設定の共通定義 ---


  // ミューテータUIのコンテナ（土台）ブロック定義
  Blockly.Blocks['move_mutatorcontainer'] = {
    init: function() {
      this.appendDummyInput()
          .appendField("Parameter");
      this.appendStatementInput('SETTINGS')
          .setCheck('move_settings_type'); // 'move_settings' ブロックのみを接続許可
      this.setColour(20);
      this.setTooltip('');
  ...
  // ミューテータUI内でパラメータを設定するためのブロック定義 (リファクタリング後)
  Blockly.Blocks['move_settings'] = {
    init: function() {
      let currentInput = this.appendDummyInput();
      // 必要であれば最初の入力行のアライメントを設定 (通常はデフォルトで左揃え)
      // currentInput.setAlign(Blockly.ALIGN_LEFT);

      MOVE_PARAM_DEFINITIONS.forEach((paramDef, index) => {
        // paramDef.newLineInMutator が true で、かつ現在の行に既にフィールドがある場合、または最初の要素でない場合に改行
        if (paramDef.newLineInMutator && (currentInput.fieldRow.length > 0 || index > 0)) {
          currentInput = this.appendDummyInput();
          // 必要であれば新しい入力行のアライメントを設定
          // currentInput.setAlign(Blockly.ALIGN_LEFT);
        }
        
        currentInput.appendField(paramDef.label) // ラベルが空文字列なら何も表示されない
                    .appendField(new Blockly.FieldDropdown(paramDef.options), paramDef.name);
      });
      this.setColour(20);
      this.setTooltip('');
      this.setPreviousStatement(true, 'move_settings_type'); 
      this.setNextStatement(false); // このブロックはスタックの最後になる
    }
  };
  ...
  Blockly.Blocks['moveL'] = {
    init: function() {
      ...
      this.hasBlockNo = true; 
      this.hasBlockFlow = true; 
      this.appendDummyInput('MAIN_DISPLAY')
          .appendField(`(${this.blockNo})`, "blockNoField")
          .appendField("moveL to")
          .appendField(new Blockly.FieldDropdown(self.options), "point_name_list") // self.options は外部で定義されている前提
          .appendField(" ", "MOVE_PARAMS"); // パラメータサマリー表示用フィールド

      // パラメータ値を内部的に保持するための非表示フィールド群
      // これらは直接ユーザーには見えないが、getFieldValue/setFieldValue でアクセスされる
      const paramsInput = this.appendDummyInput('PARAMS_FIELDS_INPUT');
      paramsInput.setVisible(false); 
      MOVE_PARAM_DEFINITIONS.forEach(paramDef => {
          paramsInput.appendField(new Blockly.FieldDropdown(paramDef.options), paramDef.name);
      });
      ...
      this.setHelpUrl(""); 
      this.originalColor = this.getColour();
      // INIT_TIMEOUT_MILLIS は外部で定義されている前提
      this.timeoutMillis = typeof INIT_TIMEOUT_MILLIS !== 'undefined' ? INIT_TIMEOUT_MILLIS : -1;
      self.addTimeoutOption(this); // self.addTimeoutOption は外部で定義されている前提
      
      this.setMutator(new Blockly.icons.MutatorIcon([], this)); 
      
      // 初期値を paramsInput のフィールドに設定 (もし init で行う場合)
      // これを行わない場合、compose や loadExtraState で初めて値が設定される
      MOVE_PARAM_DEFINITIONS.forEach(paramDef => {
        if(this.getField(paramDef.name)) { // 隠しフィールドが存在すれば
          this.setFieldValue(paramDef.default, paramDef.name);
        }
      });
      this._updateMoveParams(); // 初期表示を生成
    },

    decompose: function(workspace) {
      const containerBlock = workspace.newBlock('move_mutatorcontainer');
      containerBlock.initSvg();
      
      const settingsBlock = workspace.newBlock('move_settings');
      settingsBlock.initSvg();
      
      // 現在のブロックのフィールド値をミューテータUIのブロックに設定
      MOVE_PARAM_DEFINITIONS.forEach(paramDef => {
        // settingsBlock には paramDef.name の名前でフィールドが存在する前提
        if (settingsBlock.getField(paramDef.name) && this.getField(paramDef.name)) {
          settingsBlock.getField(paramDef.name).setValue(this.getFieldValue(paramDef.name));
        }
      });
      
      if (containerBlock.getInput('SETTINGS')) {
        containerBlock.getInput('SETTINGS').connection.connect(settingsBlock.previousConnection);
      }
      return containerBlock;
    },

    compose: function(containerBlock) {
      const settingsBlock = containerBlock.getInputTargetBlock('SETTINGS');
      if (settingsBlock) {
        // ミューテータUIの値をメインブロックの隠しフィールドに反映
        MOVE_PARAM_DEFINITIONS.forEach(paramDef => {
          if (settingsBlock.getField(paramDef.name) && this.getField(paramDef.name)) {
            this.setFieldValue(settingsBlock.getFieldValue(paramDef.name), paramDef.name);
          }
        });
      } else { 
        // ミューテータが空で閉じられた場合などはデフォルト値を設定
        MOVE_PARAM_DEFINITIONS.forEach(paramDef => {
          if (this.getField(paramDef.name)) {
            this.setFieldValue(paramDef.default, paramDef.name);
          }
        });
      }
      // this._updateMoveParams(); // setFieldValue内で呼ばれるので、ここでは不要な場合もあるが、確実性を期すなら呼ぶ
    },

    // フィールド値が変更されたときに呼び出される
    setFieldValue: function(newValue, fieldName) {
      // まず親クラスのsetFieldValueを呼び出して実際に値を設定
      Blockly.Block.prototype.setFieldValue.call(this, newValue, fieldName);

      if (fieldName === 'MOVE_PARAMS') { return; } // サマリー表示用フィールド自体の変更は無視

      // MOVE_PARAM_DEFINITIONS に含まれるフィールドかどうかを確認
      const isRelevantField = MOVE_PARAM_DEFINITIONS.some(p => p.name === fieldName);
      
      if (isRelevantField) {
        // ドラッグ中でなく、レンダリング済みの場合にサマリーを更新
        if (this.workspace && !this.workspace.isDragging() && this.rendered) {
          this._updateMoveParams();
        }
      }
    },

    _updateMoveParams: function() {
      if (!this.getField('MOVE_PARAMS') || !this.workspace) { 
        return; // 必要なフィールドやワークスペースがなければ何もしない
      }

      let summaryParts = [];
      const execModeValue = this.getFieldValue('execution_mode');
      if (execModeValue) { 
        const execModeDef = MOVE_PARAM_DEFINITIONS.find(p => p.name === 'execution_mode');
        const selectedOption = execModeDef.options.find(opt => opt[1] === execModeValue);
        summaryParts.push(`Mode:${selectedOption ? selectedOption[0] : execModeValue.toUpperCase()}`); // 表示名を使う
      } else {
        summaryParts.push('Mode:WAIT'); // フォールバック
      }
      
      const axisDefinitions = MOVE_PARAM_DEFINITIONS.filter(p => p.isAxis);
      let enabledAxesLabels = [];
      let allAxesDefinedAndFieldsExist = true; // 全ての軸定義に対応するフィールドがブロックに存在するか
      let allAxesEnabled = true;               // 全ての存在する軸フィールドが 'enable' か
      let noAxesEnabled = true;                // 全ての存在する軸フィールドが 'disable' か (一つでもenableならfalse)

      for (const axisDef of axisDefinitions) {
        if (!this.getField(axisDef.name)) { // ブロックに該当軸の隠しフィールドがなければ
          allAxesDefinedAndFieldsExist = false;
          allAxesEnabled = false; // 不明なのでALLとは言えない
          noAxesEnabled = false;  // 不明なのでNONEとは言えない
          break; 
        }
        const controlValue = this.getFieldValue(axisDef.name);
        if (controlValue === 'enable') {
          enabledAxesLabels.push(axisDef.axisLabel);
          noAxesEnabled = false; // 一つでもenableなら、noAxesEnabledはfalse
        } else { // 'disable' または他の値 (存在しないはずだが念のため)
          allAxesEnabled = false; // 一つでもdisableなら、allAxesEnabledはfalse
        }
      }
      
      if (!allAxesDefinedAndFieldsExist) {
        summaryParts.push('Axes:?'); // 軸情報が不完全
      } else if (allAxesEnabled) { // かつ全フィールドが存在
        summaryParts.push('Axes:ALL');
      } else if (noAxesEnabled && enabledAxesLabels.length === 0) { // 全てdisableで、有効なものがない
        summaryParts.push('Axes:NONE');
      } else if (enabledAxesLabels.length > 0) {
        summaryParts.push(`Axes:${enabledAxesLabels.join(',')}`);
      } else { 
        // allAxesDefinedAndFieldsExist が true で、allAxesEnabled が false で、noAxesEnabled も false (つまりenable/disable混在だがenabledAxesLabelsが0?)
        // このケースは通常発生しづらいが、フォールバックとして
        summaryParts.push('Axes:-'); 
      }
      
      const palletVal = this.getFieldValue('pallet_list');
      if (palletVal && palletVal !== 'none') {
        const palletDef = MOVE_PARAM_DEFINITIONS.find(p => p.name === 'pallet_list');
        const selectedOption = palletDef.options.find(opt => opt[1] === palletVal);
        summaryParts.push(`Pallet:${selectedOption ? selectedOption[0].replace('pallet ', '') : palletVal}`); // 表示名から "pallet " を除くなど整形
      }
      const cameraVal = this.getFieldValue('camera_list');
      if (cameraVal && cameraVal !== 'none') {
        const cameraDef = MOVE_PARAM_DEFINITIONS.find(p => p.name === 'camera_list');
        const selectedOption = cameraDef.options.find(opt => opt[1] === cameraVal);
        summaryParts.push(`Camera:${selectedOption ? selectedOption[0].replace('camera ', '') : cameraVal}`); // 表示名から "camera " を除くなど整形
      }
      
      const summaryField = this.getField('MOVE_PARAMS');
      if (summaryField) {
        summaryField.setValue(summaryParts.join(' '));
      }
    }
  };
  ...
  Blockly.Blocks['moveP'] = {
    ...
    _updateMoveParams: function() {
      ...
      Blockly.Blocks['connect_external_io'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.externalDevices || self.externalDevices.length === 0) {
            self.externalDevices = [['Reload page.', 'none']]; 
          }

          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.externalMakers || self.externalMakers.length === 0) {
            self.externalMakers = [['Reload page.', 'none']]; 
          }
          ...
          this.appendDummyInput()
              ...
              .appendField(new Blockly.FieldDropdown(self.externalDevices), "device_name") // ["deviceName","device_name"],
              .appendField("of")
              .appendField(new Blockly.FieldDropdown(self.externalMakers), "external_maker") // ["maker","maker"],
              .appendField("as")
              .appendField(new Blockly.FieldDropdown([
                                                      // ["IONo","IONo"],
                                                      ["IO\nNo.1","1"],
      ...
      Blockly.Blocks['wait_external_io_input'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.externalInputNames || self.externalInputNames.length === 0) {
            self.externalInputNames = [['Reload page.', 'none']]; 
          }
      ...
      Blockly.Blocks['set_external_io_output'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.externalOutputNames || self.externalOutputNames.length === 0) {
            self.externalOutputNames = [['Reload page.', 'none']]; 
          }
      ...
      Blockly.Blocks['set_external_io_output_during_upon'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.externalOutputNames || self.externalOutputNames.length === 0) {
            self.externalOutputNames = [['Reload page.', 'none']]; 
      ...
      Blockly.Blocks['pause_robot_upon'] = {
        init: function() {
          this.blockNo = self.blockUtilsIns.getBlockCount(this.type); 
          this.customId = this.type + '@' + this.blockNo; 
          this.hasBlockNo = true; 
      ...
      Blockly.Blocks['set_number'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.numberNames || self.numberNames.length === 0) {
            self.numberNames = [['Reload page.', 'none']]; 
          }
      ...
      Blockly.Blocks['set_flag'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.flagNames || self.flagNames.length === 0) {
            self.flagNames = [['Reload page.', 'none']]; 
          }
      ...
      Blockly.Blocks['logic_custom_flag'] = {
        init: function() {
          // 未定義の場合、デフォルトの選択肢を設定
          if (!self.flagNamesReadOnly || self.flagNamesReadOnly.length === 0) {
            self.flagNamesReadOnly = [['Reload page.', 'none']]; 
          }
          ...
          this.hasBlockFlow = false; 
          this.appendDummyInput()
              .appendField(new Blockly.FieldDropdown(self.flagNamesReadOnly), "name")
          this.setInputsInline(true);
          this.setOutput(true, "Boolean");
          this.setColour(200);
          this.setTooltip(`${this.customId}`);
      ...
      Blockly.Blocks['set_plc_bit_during'] = {
        init: function() {
          ...
          this.appendDummyInput()
              ...
              .appendField("during")
              .appendField(new Blockly.FieldDropdown(self.numberNamesReadOnly), "number_name")
              .appendField("msec");
          this.setInputsInline(true);
          this.setPreviousStatement(true, "process");
          this.setNextStatement(true, "process");
...
class BlockCode {
  ...
  defineBlockCode() {
    ...
    python.pythonGenerator.forBlock['select_robot'] = function(block, generator) {
      ...
      // システムステータス
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F460']['value'] = L.getRelay(R, 8000)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F461']['value'] = L.getRelay(R, 8001)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F462']['value'] = L.getRelay(R, 8002)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F463']['value'] = L.getRelay(R, 8003)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F464']['value'] = L.getRelay(R, 8004)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F465']['value'] = L.getRelay(R, 8005)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F466']['value'] = L.getRelay(R, 8006)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F467']['value'] = L.getRelay(R, 8007)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F468']['value'] = L.getRelay(R, 8008)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F469']['value'] = L.getRelay(R, 8009)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F470']['value'] = L.getRelay(R, 8010)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F471']['value'] = L.getRelay(R, 8011)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F472']['value'] = L.getRelay(R, 8012)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F473']['value'] = L.getRelay(R, 8013)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F474']['value'] = L.getRelay(R, 8014)" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F475']['value'] = L.getRelay(R, 8015)" + LF); 
      ...
      line_str.push(INDENT + INDENT + INDENT + INDENT + "flag_param_yaml['F485']['value'] = L.getRelay(MR, 508)" + LF); 
      // ロボット状態取得
      ...
      line_str.push(INDENT + INDENT + INDENT + INDENT + INDENT + "RAC.send_command('pauseRobot()')" + LF); 
      // 一時停止再開
      line_str.push(INDENT + INDENT + INDENT + INDENT + "L.LD(L.local_R['reset_pausing[0]']['name'], L.local_R['reset_pausing[0]']['addr'])" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + "if (L.aax & L.iix):" + LF); 
      line_str.push(INDENT + INDENT + INDENT + INDENT + INDENT + "RAC.send_command('resumeRobot()')" + LF); 
      // リセット
    ...
    python.pythonGenerator.forBlock['start_thread'] = function(block, generator) {
      ...
      logic_bits.forEach((logic_bit) => {
        ...
        if ((logic_bit === 'True') || (logic_bit === 'False')){
          ...
          // "getRelay" が含まれているなら
          if (logic_bit.includes("getRelay")) {
            const regex = /^L\.getRelay\((.*)\)$/; // "L.getRelay("で始まり、")"で終わるパターン
            const match = logic_bit.match(regex);
            // パターンにマッチし、かつ括弧の中身が取得できたら
            if (match && match[1]) {
              anpbArg = match[1];
            }
          }
      ...
      line_str.push(INDENT + INDENT + INDENT + `L.LD(${anpbArg})` + LF);
      // line_str.push(INDENT + INDENT + INDENT + `L.TMS(L.local_T['start_thread_timer[0]']['addr'], 1)` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.OUT(L.local_MR['start_thread_trigger[${block.blockNo-1}]']['name'], L.local_MR['start_thread_trigger[${block.blockNo-1}]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `#;Process:` + block.customId + LF);
      for (let i = 0; i < myBlock.survival1_addr_list.length; i++) {
        if(myBlock.survival1_addr_list[i] === 992002){
          ...
          line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['seq_step[${myBlock.survival1_addr_list[i]}]']['name'], L.local_MR['seq_step[${myBlock.survival1_addr_list[i]}]']['addr'])` + LF);
        }
      }
      line_str.push(INDENT + INDENT + INDENT + `L.ANPB(L.local_MR['start_thread_trigger[${block.blockNo-1}]']['name'], L.local_MR['start_thread_trigger[${block.blockNo-1}]']['addr'])` + LF);
      if(myBlock.reset_addr_list){
      ...
      line_str.push(INDENT + INDENT + INDENT + `L.OUT(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.MPP()` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['start_thread_trigger[${block.blockNo-1}]']['name'], L.local_MR['start_thread_trigger[${block.blockNo-1}]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.OR(L.local_MR['seq_step[${myBlock.stop1_addr}]']['name'], L.local_MR['seq_step[${myBlock.stop1_addr}]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.ANL()` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.OUT(L.local_MR['seq_step[${myBlock.stop1_addr}]']['name'], L.local_MR['seq_step[${myBlock.stop1_addr}]']['addr'])` + LF);   
      ...
      python.pythonGenerator.forBlock['moveL'] = function(block, generator) {  
        ...
        const execMode = block.getFieldValue('execution_mode');
        ...
        const vel = self.blockUtilsIns.teachingJson[point_name_str]['vel'];
        const acc = self.blockUtilsIns.teachingJson[point_name_str]['acc'];
        const dec = self.blockUtilsIns.teachingJson[point_name_str]['dec'];
        const dist = self.blockUtilsIns.teachingJson[point_name_str]['dist'];
        const stime = self.blockUtilsIns.teachingJson[point_name_str]['stime'];
        const tool = self.blockUtilsIns.teachingJson[point_name_str]['tool'];
        const posture = self.blockUtilsIns.teachingJson[point_name_str]['posture'];
        const global_override = self.blockUtilsIns.override;
      ...
      //   }
        ...
        line_str.push(INDENT + INDENT + INDENT + INDENT + `x, y, z, rx, ry, rz, vel, acc, dec, dist, stime, tool, posture = L.FB_setRobotParam(${x}, ${y}, ${z}, ${rx}, ${ry}, ${rz}, ${vel}, ${acc}, ${dec}, ${dist}, ${stime}, ${tool}, ${posture}, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, ${global_override}, program_override)` + LF);
        if (execMode === 'wait')      line_str.push(INDENT + INDENT + INDENT + INDENT + `RAC.send_command(f'moveAbsoluteLine({json.dumps({'X': x, 'Y': y, 'Z': z, 'RX': rx, 'RY': ry, 'RZ': rz})}, {json.dumps({'TOOL': 1, 'USER': 1, 'POSTURE': posture, 'VEL': vel, 'ACC': acc, 'DEC': dec})})')` + LF);
        else if (execMode === 'cont') line_str.push(INDENT + INDENT + INDENT + INDENT + `RAC.send_command(f'moveAbsoluteLineContinuous({json.dumps({'X': x, 'Y': y, 'Z': z, 'RX': rx, 'RY': ry, 'RZ': rz})}, {json.dumps({'TOOL': 1, 'USER': 1, 'POSTURE': posture, 'VEL': vel, 'ACC': acc, 'DEC': dec})})')` + LF);
        // Move命令完了待機
        line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
        line_str.push(INDENT + INDENT + INDENT + `L.ANB(L.local_MR['seq_step[${myBlock.stop1_addr}]']['name'], L.local_MR['seq_step[${myBlock.stop1_addr}]']['addr'])` + LF);
        line_str.push(INDENT + INDENT + INDENT + `if ((L.aax & L.iix) and (RAC.connected)):` + LF);
        if (execMode === 'wait')      line_str.push(INDENT + INDENT + INDENT + INDENT + `RAC.send_command(f'waitArrive({json.dumps({'X': x, 'Y': y, 'Z': z, 'RX': rx, 'RY': ry, 'RZ': rz})}, {dist})')` + LF);
        else if (execMode === 'cont') line_str.push(INDENT + INDENT + INDENT + INDENT + `RAC.send_command(f'waitContinuousPointInput({json.dumps({'X': x, 'Y': y, 'Z': z, 'RX': rx, 'RY': ry, 'RZ': rz})}, {dist})')` + LF);        
        // 静定時間待機
        line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
        line_str.push(INDENT + INDENT + INDENT + `L.AND(robot_status['arrived'])` + LF);
        ...
        // ロボット動作中
        line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
        line_str.push(INDENT + INDENT + INDENT + `L.OUT(L.local_MR['robot_busy[${myBlock.index}]']['name'], L.local_MR['robot_busy[${myBlock.index}]']['addr'])` + LF);
        line_str.push(INDENT + INDENT + LF);
        return line_str.join("");
        };
      ...
      //   }
        ...
        line_str.push(INDENT + INDENT + INDENT + INDENT + `x, y, z, rx, ry, rz, vel, acc, dec, dist, stime, tool, posture = L.FB_setRobotParam(${x}, ${y}, ${z}, ${rx}, ${ry}, ${rz}, ${vel}, ${acc}, ${dec}, ${dist}, ${stime}, ${tool}, ${posture}, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, ${global_override}, program_override)` + LF);
        if (execMode === 'wait')      line_str.push(INDENT + INDENT + INDENT + INDENT + `RAC.send_command(f'moveAbsolutePtp({json.dumps({'X': x, 'Y': y, 'Z': z, 'RX': rx, 'RY': ry, 'RZ': rz})}, {json.dumps({'TOOL': 1, 'USER': 1, 'POSTURE': posture, 'VEL': vel, 'ACC': acc, 'DEC': dec})})')` + LF);
        else if (execMode === 'cont') line_str.push(INDENT + INDENT + INDENT + INDENT + `RAC.send_command(f'moveAbsolutePtpContinuous({json.dumps({'X': x, 'Y': y, 'Z': z, 'RX': rx, 'RY': ry, 'RZ': rz})}, {json.dumps({'TOOL': 1, 'USER': 1, 'POSTURE': posture, 'VEL': vel, 'ACC': acc, 'DEC': dec})})')` + LF);
        // Move命令完了待機
        line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
        line_str.push(INDENT + INDENT + INDENT + `L.ANB(L.local_MR['seq_step[${myBlock.stop1_addr}]']['name'], L.local_MR['seq_step[${myBlock.stop1_addr}]']['addr'])` + LF);
        line_str.push(INDENT + INDENT + INDENT + `if ((L.aax & L.iix) and (RAC.connected)):` + LF);
  ...
  python.pythonGenerator.forBlock['controls_if'] = function(block, generator) {
    ...
    line_str.push(INDENT + INDENT + INDENT + `#;Pre-Process:` + block.customId + LF);
    // IFブロックの分岐数を確認
    const getSafeValue = (block, inputName, order, defaultValue = '0') => {
      if (block.getInput(inputName)) {
        // 入力が存在する場合のみ valueToCode を試みる
        return generator.valueToCode(block, inputName, order) || defaultValue;
      }
      // 入力が存在しない場合はデフォルト値を返す
      return defaultValue;
    };
    // 上記ヘルパー関数を使用
    const IF0 = getSafeValue(block, 'IF0', generator.ORDER_ATOMIC);
    const IF1 = getSafeValue(block, 'IF1', generator.ORDER_ATOMIC);
    const IF2 = getSafeValue(block, 'IF2', generator.ORDER_ATOMIC);
    const IF3 = getSafeValue(block, 'IF3', generator.ORDER_ATOMIC);
    const IF4 = getSafeValue(block, 'IF4', generator.ORDER_ATOMIC);
    const IF5 = getSafeValue(block, 'IF5', generator.ORDER_ATOMIC);
    const IF6 = getSafeValue(block, 'IF6', generator.ORDER_ATOMIC);
    const IF7 = getSafeValue(block, 'IF7', generator.ORDER_ATOMIC);
    const IF8 = getSafeValue(block, 'IF8', generator.ORDER_ATOMIC);
    ...
    // 内包ブロックのコード生成
    var branches = ['DO0', 'DO1', 'DO2', 'DO3', 'DO4', 'DO5', 'DO6', 'DO7', 'DO8', 'ELSE']
      .map(label => {
        // 入力がブロックに存在するかを確認
        if (block.getInput(label)) {
          // 通常は空文字列 '' や 'pass\n' (Pythonの場合) などを返す
          return generator.statementToCode(block, label);
        }
        // 入力が存在しない場合は、空文字列を返す (または他の適切なデフォルト値)
        return '';
      })
      // 空の分岐にる多数の改行を削除
      .filter(code => code && code.trim() !== '' && code.trim() !== 'pass') // 空や 'pass' だけの行を除外
      .join('\n');
  ...
  python.pythonGenerator.forBlock['raise_error_upon'] = function(block, generator) {
    ...
    python.pythonGenerator.forBlock['pause_robot'] = function(block, generator) {
      //////////////////////////////////////////////////
      // アドレス参照
      //////////////////////////////////////////////////   
      const myBlock = self.getBlockAddr(block.customId); 
      ...
      line_str.push(INDENT + INDENT + INDENT + `L.MPP()` + LF);
      // line_str.push(INDENT + INDENT + INDENT + `L.LDB(MR, 304)` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_R['reset_pausing[0]']['name'], L.local_R['reset_pausing[0]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.ANPB(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `L.LDP(R, 7801)` + LF); 
      line_str.push(INDENT + INDENT + INDENT + `L.ORB(R, 7800)` + LF); 
      line_str.push(INDENT + INDENT + INDENT + `L.ANL()` + LF);
      ...
      line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_R['pausing[0]']['name'], L.local_R['pausing[0]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `if (L.aax & L.iix):` + LF);
  ...
  python.pythonGenerator.forBlock['pause_robot_upon'] = function(block, generator) {
    //////////////////////////////////////////////////
    // アドレス参照
    //////////////////////////////////////////////////   
    const myBlock = self.getBlockAddr(block.customId); 
  ...
  python.pythonGenerator.forBlock['connect_external_io'] = function(block, generator) {
    ...
    const ioNo = Number(block.getFieldValue('io_no')) % 100;
    ...
    if (externalMaker === "contec"){
      // line_str.push(INDENT + INDENT + INDENT + INDENT + `external_io_instance[${ioNo}-1] = io_api_${ioNo}` + LF);
      line_str.push(INDENT + INDENT + INDENT + INDENT + `external_io_instance[${ioNo}-1] = cdio_api.ContecDioDevice("${deviceName}")` + LF);
      line_str.push(INDENT + INDENT + INDENT + INDENT + `external_io_connected[${ioNo}-1] = external_io_instance[${ioNo}-1].connect()` + LF);
    }
    // error
    line_str.push(INDENT + INDENT + INDENT + `#;error:` + block.customId + LF);
    line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
  ...
  python.pythonGenerator.forBlock['set_external_io_output_upon'] = function(block, generator) {
    ...
    // msecタイマーなら
      ...
      line_str.push(INDENT + INDENT + INDENT + `L.LD(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
      if (outState === '100msec')         line_str.push(INDENT + INDENT + INDENT + `L.ANB(L.local_T['100msec_timer[0]']['name'], L.local_T['100msec_timer[0]']['addr'])` + LF);
      else if (outState === '300msec')    line_str.push(INDENT + INDENT + INDENT + `L.ANB(L.local_T['300msec_timer[0]']['name'], L.local_T['300msec_timer[0]']['addr'])` + LF);
      else if (outState === '500msec')    line_str.push(INDENT + INDENT + INDENT + `L.ANB(L.local_T['500msec_timer[0]']['name'], L.local_T['500msec_timer[0]']['addr'])` + LF);
      else if (outState === '1000msec')   line_str.push(INDENT + INDENT + INDENT + `L.ANB(L.local_T['1000msec_timer[0]']['name'], L.local_T['1000msec_timer[0]']['addr'])` + LF);
      line_str.push(INDENT + INDENT + INDENT + `if (L.aax & L.iix):` + LF);
      // line_str.push(INDENT + INDENT + INDENT + `else:` + LF);
      line_str.push(INDENT + INDENT + INDENT + INDENT + `if (hasattr(external_io_instance[${ioNo}-1], 'get_input')):` + LF);
      line_str.push(INDENT + INDENT + INDENT + INDENT + INDENT + `external_io_instance[${ioNo}-1].set_output_off(${outPinNo})` + LF);       
  ...
  python.pythonGenerator.forBlock['set_external_io_output_during_upon'] = function(block, generator) {
    //////////////////////////////////////////////////
    // アドレス参照
    //////////////////////////////////////////////////   
    const myBlock = self.getBlockAddr(block.customId); 
    ...
    line_str.push(INDENT + INDENT + INDENT + `#;action:` + block.customId + LF);   
    // action
    if      (triggerSate === 'rising')  line_str.push(INDENT + INDENT + INDENT + `L.LDP(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
    line_str.push(INDENT + INDENT + INDENT + `if (L.aax & L.iix):` + LF);
    line_str.push(INDENT + INDENT + INDENT + INDENT + `if (hasattr(external_io_instance[${ioNo}-1], 'get_input')):` + LF);
    if      (outState === 'on')  line_str.push(INDENT + INDENT + INDENT + INDENT + INDENT + `external_io_instance[${ioNo}-1].set_output_on(${outPinNo})` + LF); 
    else if (outState === 'off') line_str.push(INDENT + INDENT + INDENT + INDENT + INDENT + `external_io_instance[${ioNo}-1].set_output_off(${outPinNo})` + LF); 
    ...
    line_str.push(INDENT + INDENT + INDENT + `L.TMS(L.local_T['block_timer1[${myBlock.index}]']['addr'], wait_msec=${timer})` + LF);
    line_str.push(INDENT + INDENT + INDENT + `L.LDP(L.local_T['block_timer1[${myBlock.index}]']['name'], L.local_T['block_timer1[${myBlock.index}]']['addr'])` + LF);
    line_str.push(INDENT + INDENT + INDENT + `L.ORF(L.local_MR['seq_step[${myBlock.start_addr}]']['name'], L.local_MR['seq_step[${myBlock.start_addr}]']['addr'])` + LF);
    line_str.push(INDENT + INDENT + INDENT + `if (L.aax & L.iix):` + LF);
    line_str.push(INDENT + INDENT + INDENT + INDENT + `if (hasattr(external_io_instance[${ioNo}-1], 'get_input')):` + LF);
