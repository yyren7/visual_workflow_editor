  ...
  while True:
    try:
      ...
      if ((L.aax & L.iix) and (RAC.connected)):
        ...
        offset_ry = 0
        offset_rz = 0
        x, y, z, rx, ry, rz, vel, acc, dec, dist, stime, tool = L.FB_setRobotParam(6.798, -367.197, 212.283, 178.97, -2.454, -73.874, 100.0, 100.0, 100.0, 0.1, 0.0, 0.0, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, 100)
      ...
      if ((L.aax & L.iix) and (RAC.connected)):
        ...
        offset_ry = 0
        offset_rz = 0
        x, y, z, rx, ry, rz, vel, acc, dec, dist, stime, tool = L.FB_setRobotParam(156.115, -367.197, 212.269, 178.97, -2.451, -73.871, 100.0, 100.0, 100.0, 0.1, 0.0, 0.0, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, 100)
        RAC.send_command(f'moveAbsolutePtp({x}, {y}, {z}, {rx}, {ry}, {rz}, {vel}, {acc}, {dec}, {int(tool)})')
      L.LD(L.local_MR['seq_step[4]']['name'], L.local_MR['seq_step[4]']['addr'])
      L.ANB(L.local_MR['seq_step[2004]']['name'], L.local_MR['seq_step[2004]']['addr'])
      if ((L.aax & L.iix) and (RAC.connected)):
